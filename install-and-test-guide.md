...
- **Your shared data syncs through Cloudflare** once Couple sync is connected in Profile → **Account & data**. On reinstall or a new phone, sign in with the same allowed Google account so Mesa can restore from the cloud copy.
...
- Switch **Elena ⇄ Andrea** (top right). Your breakfasts differ (solo meals), dinner is shared — does that split feel right? Note the non-saturated-fat estimate and saturated-fat line under the ring.
...
- On **Week**, check that the 7-day meal summaries are visible without scrolling past large cards. Use the top toolbar: **Shopping** opens the list for the selected week, **Re-balance** previews conservative swaps for the selected week, and **Regenerate** keeps pinned/logged meals before replacing the rest.
...
- Open Profile → **How Mesa plans**. Check that Guidelines, Estimates and Mesa rules are clearly distinct, sources open correctly, and the limits say what Mesa does not assess. Then tap a meal → recipe: **"Why Mesa picked this"** should explain planning fit without a health-condition claim. Try the servings steppers ("Elena 1× · Andrea 1.5×") — ingredient quantities rescale live.
- On **Week**, tap **Week quality** to expand the computed averages/coverage chips, then tap a day — it expands to that day's meals. The nutrition drawer should feel available, not like a blocker before the plan.
