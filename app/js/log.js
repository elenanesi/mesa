...
function upsertLogEntry(dateISO, personKey, entry){
  ...
  if(entry.kind === 'plan' && entry.slot){
    ...
    if(idx !== -1){
      ...
      if(arr[idx].eatenOut) entry.eatenOut = true; // and keep the eaten-out flag too — a swap/portion correction (chooseSwapRecipe, etc.) must not silently un-mark a slot already logged eaten out
