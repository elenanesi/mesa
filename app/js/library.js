...
function renderLibraryHub(){
  ...
  el.innerHTML =
    ...
    // Profile / settings was previously reachable ONLY from the Today avatar — a dead-end from
    // every other tab. Surface it here too so settings are always ≤2 taps from anywhere.
    + '<div class="altrow" onclick="go(\'profile\')" style="margin-top:14px"><div class="ae">⚙️</div><div class="at"><div class="an">Profile &amp; settings</div><div class="ad">Your basics, plan, food preferences and account</div></div></div>'
    + '</div>';
}
...
// (openAddMenu was retired — the centre + FAB now goes straight to the Log screen, the daily
// one-tap action; authoring/scan live in the Library hub below.)
...
function buildFoodDetailMarkup(id){
  ...
  const actions = '<div class="ingredient-detail-actions">'
    + (foodCanBePantryBaselined(id) ? '<button class="ingredient-detail-action ingredient-detail-action-primary" data-act="pantry">🥫 Add to pantry</button>' : '')
    + '<button class="ingredient-detail-action" data-act="edit">✎ Edit</button>'
    + (isEdited ? '<button class="ingredient-detail-action ingredient-detail-action-quiet" data-act="reset">↺ Reset</button>' : '')
    + (isCustom ? '<button class="ingredient-detail-action ingredient-detail-action-danger" data-act="delete">Delete</button>' : '')
    + '</div>';
